...
async function main() {
	...
	const walletContract = WalletContract.create(
		client,
		WalletV4Source.create({ publicKey: walletKey.publicKey, workchain })
	);
	...
	for (const rootContract of rootContracts) {
		...
		if (await client.isContractDeployed(newContractAddress)) {
			...
			console.log(
				` - New contract balance is now ${fromNano(contractBalance)} TON, make sure it has enough to pay rent`
			);
		} else {
